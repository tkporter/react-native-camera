      ...
      [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:[self.stillImageOutput connectionWithMediaType:AVMediaTypeVideo] completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        if (imageDataSampleBuffer) {
          ...
          if ([options objectForKey:@"rotation"]) {
            ...
            UIImage *img = [[UIImage alloc]initWithCGImage:rotatedCGImage];
            CGImageRelease(rotatedCGImage);
            UIImage *resized = [self resizeImage:img withScale:.15f];
            rotatedCGImage = CGImageCreateCopy(resized.CGImage);
...
-(UIImage *)resizeImage:(UIImage *)image withScale:(float)scale {
...
-(UIImage *)resizeImage :(UIImage*)img withScale:(float)scale
{
    CGRect rect = CGRectMake(0, 0, img.size.width * scale, img.size.height * scale);
